...
bool equal(unordered_set< T >& left, std::unordered_set< T >& right)
{
	...
	{
		cout << left.size() << " "<<right.size() << " size Error" << endl;
		return false;
	}

	if (left.bucket_count() != right.bucket_count())
	{
		cout << left.bucket_count() << " " << right.bucket_count() << endl;
		cout << "bucket Count Error" << endl;
		return false;
	}
	...
	for (size_t bucketNo = 0; bucketNo < right.bucket_count(); bucketNo++)
	{
		...
		{
			cout << left.bucket_size(bucketNo) << " " << right.bucket_size(bucketNo) << endl;
			cout << "bucket Size Error" << endl;
			return false;
		}
		...
		if (rightBucketFirst != rightListHead)
		{
			if (rightBucketFirst != rightBucketLast)
			{
				...
				while (rightIt != rightBucketLast)
				{
					if (*leftIt != *rightIt)
					{
						cout << "in bucket order error" << endl;
						return false;
					}
