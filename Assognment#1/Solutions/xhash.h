...
struct HashVec
{
	...
	void assignGrow(const size_type cells, const value_type val)
	{
		...
		myData.myEnd = myData.myLast = myData.myFirst + cells;

		for (auto it = myData.myFirst; it != myData.myEnd; it++)
			*it = val;
	}
...
public:
	...
	size_type bucket_size(size_type n) const
	{
		...
		iterator it = myVec.myData.myFirst[n * 2];
		size_type count = 1;

		for (; it != myVec.myData.myFirst[n * 2 + 1]; it++)
			count++;
	...
	void erase(const key_type& keyVal)
	{
		...
		if (it != myList.end()) // found
		{
			...
			size_type buc = bucket(keyVal);
			auto bucketStart = &myVec.myData.myFirst[2 * buc];
			auto bucketFinish = bucketStart+1;


			if (*bucketStart == *bucketFinish) // 如果bucket只有一個元素
				*bucketFinish = *bucketStart = myList.end();
			else // bucket 有兩個元素以上
				if (it == *bucketStart) // bucket 在最前面
					(*bucketStart)++;
				else if (it == *bucketFinish) // bucket 在最後面
					(*bucketFinish)--;
...
private:
	...
	{
		size_type bucNo = bucket(val);

		if (myVec.myData.myFirst[2 * bucNo] == myList.end())
		{ 
			myList.insert(myVec.myData.myFirst[2 * bucNo], val);
			myVec.myData.myFirst[2 * bucNo + 1] = myVec.myData.myFirst[2 * bucNo] = find(val);
		}
		else
		{
			myList.insert(myVec.myData.myFirst[2 * bucNo], val);
			myVec.myData.myFirst[2 * bucNo] = find(val);
		}
